...
class TestOnFireSchema {
	constructor() {
		this.req = new TestOnFireReqSchema();
		this.res = new TestOnFireResSchema();
	}
}
...
function translator (result) {
	...
	const apidocRequestArray = JSON.parse(result.data.toString());

	let testOnFireSchema = new TestOnFireSchema();

	if (!_.isArray(apidocRequestArray)) {
		console.log('apidoc 数据解析出错');
		return;
	}

	// console.dir(apidocRequestArray, {depth: 10});
	
	for (let i=0; i<apidocRequestArray.length; i++) {

		let apidocRequestItem 	= apidocRequestArray[i];

		let reqestSchema 				= {};
		reqestSchema.httpMethod = apidocRequestItem.type;
		reqestSchema.url 				= apidocRequestItem.url;
		reqestSchema.name				= apidocRequestItem.name;

		let returnSchema				= {};
		returnSchema.httpMethod = apidocRequestItem.type;
		returnSchema.url 				= apidocRequestItem.url;
		returnSchema.name				= apidocRequestItem.name;

		let requestParams;
		let returnParams;
		try {
			requestParams = apidocRequestItem.parameter.fields.Parameter;
		} catch (e) {
			if (e) {
				consoel.log('解析apidoc request参数出错', e);
			}
		}
		
		if (_.isObject(requestParams)) {
			// reqestSchema.schema = schemas;
			reqestSchema.schema = schemaCreator(requestParams);
		}
		
		try {
			returnParams = apidocRequestItem.success.fields['Success 200'];
		} catch (e) {
			consoel.log('解析apidoc return参数出错', e);
		}

		if (_.isObject(returnParams)) {
			returnSchema.schema = schemaCreator(returnParams);
		}

		testOnFireSchema['req'][reqestSchema.httpMethod][reqestSchema.url] 	= reqestSchema['schema'];
		testOnFireSchema['res'][returnSchema.httpMethod][returnSchema.url] 	= returnSchema['schema'];
	}

	console.dir(testOnFireSchema, {depth: 10});
